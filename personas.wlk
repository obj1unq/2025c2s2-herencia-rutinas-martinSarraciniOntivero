...
class PersonaSedentaria inherits Persona{
 ...
 override method kilosPorCalorias(){
  ...
  override method ejercitarRutina(rutina){
    self.validarPeso(rutina)
    peso = self.cuantoPesoPierdeConRutina(rutina)
  }
  override method pesoQuePierdeConRutina(rutina){
    self.validarPeso(rutina)
    return (self.cuantoPesoPierdeConRutina(rutina)).truncate(3)
  }
  override method cuantoPesoPierdeConRutina(rutina){
    return peso - (rutina.caloriasBajadas(self.tiempo()) / self.kilosPorCalorias())
  }
}
...
class PersonaAtleta inherits Persona{
 ...
 override method kilosPorCalorias(){
  ...
  override method cuantoPesoPierdeConRutina(rutina){
    return peso - ((rutina.caloriasBajadas(self.tiempo()) / self.kilosPorCalorias()) - 1)
  }
  override method pesoQuePierdeConRutina(rutina){
    self.validarPeso(rutina)
...
///////////////////////////////////////////////////////////
//solucion vieja, funciona pero no me gusta :(
/*
class Persona{
  var property peso = 0 
  method ejercitarRutina(rutina){
